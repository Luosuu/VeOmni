...
def use_quack_grouped_gemm() -> None:
    global gemm

    gemm = _quack_gemm


def use_torch_scaled_grouped_gemm() -> None:
    global gemm

    from .torch_scaled_grouped_gemm import torch_scaled_grouped_gemm

    gemm = torch_scaled_grouped_gemm
...
class QuackFusedMoeExpertFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        fc1_1_output = gemm_impl(hidden_states, fc1_1_w_t, cu_seqlens_m=cu_seqlens_m, A_idx=A_idx)

        # fc1_2: [T*topk, I]
        fc1_2_output = gemm_impl(hidden_states, fc1_2_w_t, cu_seqlens_m=cu_seqlens_m, A_idx=A_idx)
        ...
        fc2_output = gemm_impl(fc1_weighted_output, fc2_w_t, cu_seqlens_m=cu_seqlens_m)
    ...
    def backward(ctx, grad_output):
        ...
        grad_fc1_weighted_output = gemm_impl(grad_fc2_output, fc2_weight, cu_seqlens_m=cu_seqlens_m)
        ...
        if fc2_weight.requires_grad:
            grad_fc2_weight = gemm_impl(grad_fc2_output.T, fc1_weighted_output, cu_seqlens_k=cu_seqlens_m)
        ...
        grad_scatter_output_2 = gemm_impl(grad_fc1_2_output, fc1_2_weight, cu_seqlens_m=cu_seqlens_m)
        ...
        grad_scatter_output_1 = gemm_impl(grad_fc1_1_output, fc1_1_weight, cu_seqlens_m=cu_seqlens_m)
        ...
        if fc1_2_weight.requires_grad:
            grad_fc1_2_weight = gemm_impl(grad_fc1_2_output.T, scatter_output, cu_seqlens_k=cu_seqlens_m)
        ...
        if fc1_1_weight.requires_grad:
            grad_fc1_1_weight = gemm_impl(grad_fc1_1_output.T, scatter_output, cu_seqlens_k=cu_seqlens_m)
...
class MergedFc1QuackFusedMoeExpertFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        fc1_output = gemm_impl(hidden_states, fc1_1_2_w_t, cu_seqlens_m=cu_seqlens_m, A_idx=A_idx)
    ...
    def backward(ctx, grad_output):
        ...
        grad_scatter_output = gemm_impl(grad_fc1_output, fc1_1_2_weight, cu_seqlens_m=cu_seqlens_m)

        # Step 4 wgrad: grad_fc1_output.T @ scatter_output → [E, 2I, H]
        grad_fc1_1_2_weight = None
        if fc1_1_2_weight.requires_grad:
            scatter_output = moe_scatter(hidden_states, scatter_index)
            grad_fc1_1_2_weight = gemm_impl(grad_fc1_output.T, scatter_output, cu_seqlens_k=cu_seqlens_m)
            del scatter_output
        del grad_fc1_output
...
class EPQuackGroupGemm(torch.autograd.Function):
    ...
    def forward(
        ...
        fc1_1_output = gemm_impl(permute_tokens, fc1_1_w_t, cu_seqlens_m=cu_seqlens_m)
        fc1_2_output = gemm_impl(permute_tokens, fc1_2_w_t, cu_seqlens_m=cu_seqlens_m)

        fc1_1_activation = torch.ops.aten.silu(fc1_1_output)
        fc1_result = fc1_1_activation * fc1_2_output

        fc2_output = gemm_impl(fc1_result, fc2_w_t, cu_seqlens_m=cu_seqlens_m)
    ...
    def backward(ctx, grad_output):
        ...
        # dgrad fc2
        grad_fc1_result = gemm_impl(grad_output, fc2_weight, cu_seqlens_m=cu_seqlens_m)

        # wgrad fc2
        grad_fc2_weight = None
        if fc2_weight.requires_grad:
            grad_fc2_weight = gemm_impl(grad_output.T, fc1_result, cu_seqlens_k=cu_seqlens_m)
        del fc1_result
        ...
        grad_fc1_2_weight = None
        if fc1_2_weight.requires_grad:
            grad_fc1_2_weight = gemm_impl(grad_fc1_2_output.T, permute_tokens, cu_seqlens_k=cu_seqlens_m)
        del grad_fc1_2_output
        ...
        grad_fc1_1_weight = None
        if fc1_1_weight.requires_grad:
            grad_fc1_1_weight = gemm_impl(grad_fc1_1_output.T, permute_tokens, cu_seqlens_k=cu_seqlens_m)
...
class EPMergedFc1QuackGroupGemm(torch.autograd.Function):
    ...
    def forward(
        ...
        fc1_output = gemm_impl(permute_tokens, fc1_1_2_w_t, cu_seqlens_m=cu_seqlens_m)
    ...
    def backward(ctx, grad_output):
        ...
        grad_permute_tokens = gemm_impl(grad_fc1_output, fc1_1_2_weight, cu_seqlens_m=cu_seqlens_m)

        # single wgrad for merged fc1
        grad_fc1_1_2_weight = None
        if fc1_1_2_weight.requires_grad:
            grad_fc1_1_2_weight = gemm_impl(grad_fc1_output.T, permute_tokens, cu_seqlens_k=cu_seqlens_m)
        del grad_fc1_output
...
def torch_scaled_grouped_gemm_fused_moe_forward(
    num_experts: int,
    routing_weights: torch.Tensor,
    selected_experts: torch.Tensor,
    hidden_states: torch.Tensor,
    ...
    return quack_gemm_fused_moe_forward(
        ...
        fc2_weight,
        fc1_1_2_weight,
        gemm_impl=torch_scaled_grouped_gemm,
    )
