...
@dataclass
class LiberoActionCollator(DataCollator):
    """Data collator for LIBERO robotic action prediction with Qwen3-VL.

    Extends ``OmniDataCollatorWithPadding`` logic to handle the extra fields
    produced by ``process_libero_sample_qwen3_vl``:

    - ``observation_state`` — padded per-sample and stacked into ``(B, state_dim)``.
    - ``labels`` — action targets stacked into ``(B, pred_len, action_dim)``.

    All VLM fields (pixel_values, image_grid_thw, input_ids, attention_mask,
    position_ids, image_mask, video_mask) are handled identically to
    ...
    # Fields that have uniform shape across samples and can be directly stacked.
    stack_features: Dict[str, None] = field(
        default_factory=lambda: {
            "observation_state": None,
            "labels": None,
        },
        metadata={"help": "features to stack (same shape across samples)."},
    )
    ...
    def __call__(self, features: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for key in list(batch.keys()):
            if key in self.concat_features:
                batch[key] = torch.cat(batch[key], dim=self.concat_features[key])
            elif key in self.padding_features:
                pad_list = batch[key]
                pad_value = self.padding_features.get(key, 0)
                if key == "position_ids" and len(batch[key][0].shape) == 2:
                    pad_list = [item.transpose(0, 1) for item in batch[key]]
            ...
            elif key in self.stack_features:
